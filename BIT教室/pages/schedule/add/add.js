...
Page({
  ...
  done:function()
  {
    ...
    wx.getStorage({
      ...
      success: function (res) {
        ...
        schedule.push([title,year,month,date,day,hour,minute,pic,schedule.length]);
        wx.setStorage({
          key: "schedule",
          data: schedule
        })
  ...
  onLoad: function (options) {
    ...
    list[count]='今天';
    var value = [count, now.getHours(), parseInt(now.getMinutes() / 5)+1];
    this.setData({
      days:list,
      hours:hours,
      minutes:minutes,
      ...
      day: now.getDay(),
      hour: now.getHours(),
      minute: minutes[(5-(now.getMinutes()%5)+now.getMinutes())/5]
    })
    this.fix(value);
  },
  fix:function(value)
